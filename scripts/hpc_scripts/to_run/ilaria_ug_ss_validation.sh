...
python3.7 -m pvt_model --initial-month 9 --location system_data/london_ilaria/ \
    ...
    --output output_files/june_week_1/ilaria_unglazed_steady_state_runs \
    --x-resolution 31 --y-resolution 11 --decoupled --steady-state \
    --steady-state-data-file \
    system_data/steady_state_data/ilaria_experimental_unglazed_validation_runs.yaml \
    --layers pv a p f --portion-covered 0.65 --skip-analysis
