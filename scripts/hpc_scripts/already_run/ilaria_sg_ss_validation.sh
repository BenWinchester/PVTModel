...
echo -e "Carrying out single-glazed Ilaria analysis based on thesis."
python3.7 -m pvt_model --initial-month 9 --location system_data/london_ilaria/ \
    --pvt-data-file system_data/pvt_panels/ilarias_experimental_panel.yaml \
    --output output_files/june_week_1/ilaria_single_glazed_steady_state_runs \
    --x-resolution 31 --y-resolution 11 --decoupled --steady-state \
    --steady-state-data-file \
    system_data/steady_state_data/ilaria_experimental_single_glazed_validation_runs.yaml \
    --layers g pv a p f --portion-covered 0.65 --skip-analysis
